# Plan: financing a ~£1,982 solicitor payment at 0% interest

Context (2026-07-21): solicitor invoice is **£1,982**, payable by card. Plan
is to route it through NatWest and M&S so the whole amount sits on 0%
interest, rather than paying standard purchase rates (24.9%–39.94%
depending on card).

**Status: EXECUTED 2026-07-28 — but it did NOT achieve 0%. See the
correction immediately below before relying on anything in this note.**

> ## 🚨 Correction (17 Sept 2026): the plan failed its own objective
>
> This note records the outcome as "£1,420.56 at 0% + £82.00 unprotected",
> with the £82 expected to cost "roughly £1–2 in interest". **Both are
> wrong.** The 16 Aug 2026 NatWest statement
> ([[2026-08-16_natwest-0354-statement.pdf]]) shows the **entire £1,502.56
> under *Purchases* at 26.436%**, with no promotional balance at all — and
> **£52.55 of interest was charged on 15 Sept 2026**.
>
> **What actually happened:** the £1,900 transferred in from M&S on 28 Jul
> **repaid the 0% balance-transfer balance outright** (taking the card
> £479.44 into credit), and the £1,982 solicitor charge then landed as
> entirely **new purchase debt** at the full rate. The M&S 0% capacity was
> therefore spent repaying money that was *already* at 0%, while the genuinely
> new, expensive borrowing was left sitting on NatWest.
>
> Also wrong above: the claim that Atkins Dellow's system capped the payment
> at £1,900. The statement shows a **single £1,982.00 charge** — the full
> invoice went on the card. The "£82 stranded" framing came from comparing
> that charge against the £1,900 transferred away, not from a payment cap.
>
> ### The specific design error, so it isn't repeated
>
> **A balance transfer arrives at the receiving card as a payment, and
> payments are allocated by the card issuer — you cannot earmark one for a
> purchase.** This plan assumed £1,900 transferred *off* NatWest would carry
> the solicitor purchase away with it. It couldn't. Under Consumer Credit Act
> allocation the incoming £1,900 went to the balance that existed at that
> moment, which was the **0% balance transfer** — the only balance on the
> card when it landed.
>
> The red flag that should have been caught at planning time: **£1,900 was
> transferred against a balance of only £1,420.56.** Transferring more than
> the balance you are protecting guarantees the promotional balance is wiped
> out and the surplus (£479.44) becomes a credit — which is exactly what the
> statement shows. The protected balance was destroyed to no purpose, the
> £66.31 transfer fee bought 0% on money already at 0%, and the genuinely new
> £1,982 of borrowing stayed behind at 26.436%.
>
> **Net position, stated fairly:** this is still slightly better than doing
> nothing (£1,502.56 left at full rate rather than the full £1,982), but far
> short of the intended £82 — roughly **£1,420 more at 26.436% than
> planned**.
>
> **Rule for next time:** write down the intended end state *per balance
> type*, never transfer more than the balance being protected, and **verify
> against the next statement's SUMMARY OF BALANCES** rather than assuming
> execution matched intent. See [[NatWest Mastercard]] and
> [[2026-08-16_natwest-statement-0pc-gone]].
>
> The M&S side of the plan is unaffected and still on track. The NatWest side
> needs rescuing — the Tesco Clubcard 0% BT window (closes **9 Oct 2026**)
> could absorb the balance for ~£63 of fee. Crucially, **that transfer is
> safe in a way this one wasn't**: NatWest now has *no* promotional balance
> left to destroy, so an incoming transfer can only land against the 26.436%
> debt. Transfer no more than the balance outstanding on the day.

## What actually happened

1. **Paid Atkins Dellow via phone** — their card system only allowed £1,900
   on the NatWest Mastercard, not the full £1,982. **Rupert decided not to
   bank-transfer the remaining £82 separately** — it's being left as a
   standard-rate balance on NatWest rather than settled directly.
2. **Balance-transferred £1,900 from NatWest to M&S** (M&S's own stated
   limit on the day, below the theoretical £1,932.55 max) — 0% for 12
   months, fee 3.49%.
3. Since the £82 wasn't paid separately, it **does sit stranded on
   NatWest at standard rate** — same shape as the original plan's ~£49.45
   stranded remainder, just a bigger amount. Not a real problem: NatWest's
   existing £300/month direct debit already exceeds the minimum, and UK
   payment-allocation rules (Consumer Credit Act) require any surplus above
   the minimum to clear the highest-interest balance first — so the £82
   should clear itself within the first payment cycle or two, at the cost
   of roughly £1–2 in interest. Decided 28 Jul: not worth a separate
   transfer for that.
4. **M&S Direct Debit Amend Form signed 28/07/2026**, changed from Minimum
   to **Fixed £175.00/month** (card ending 0645, statement date 13/03, due
   date 01/08).

Why not transfer to NatWest instead of via it: NatWest's 0% offer was a
one-time intro deal from account opening (16 Feb 2026) — no evidence of a
standing repeat offer, so new transfers in now would likely hit standard
26.436% APR, not 0%. Why not charge M&S directly: that would sit at M&S's
*purchase* rate (24.9%), since the 0% offer only applies to balances
transferred in, not to spending on the card itself.

## Actual balances after execution

| Card | Balance before | Balance after | Notes |
|---|---|---|---|
| NatWest Mastercard | £1,420.56 (0% BT) | £1,502.56 (£1,420.56 0% BT + £82.00 unprotected) | £1,900 purchase + £1,900 transfer out, £82 left stranded |
| M&S | £0.00 | **£1,966.31** (£1,900 + £66.31 fee, 3.49%) | 0% window: **28 Jul 2026 → 28 Jul 2027**. £33.69 headroom left under £2,000 limit |
| Capital One | £0.00 | unchanged | — |
| Tesco Clubcard | £74.43 | unchanged | — |

The £82 remainder of the solicitor payment was **not** paid separately —
Rupert decided to leave it on NatWest (see point 3 above).

## Repayment plan to hit zero interest

- **M&S — £175.00/month fixed** (DD amend form signed 28 Jul 2026). Clears
  £1,966.31 in ≈11.2 months — comfortably inside the 12-month 0% window,
  ~1 month's buffer before the 28 Jul 2027 deadline.
- **NatWest — left as-is, no top-up.** Existing £300/month direct debit
  continues unchanged. The £82 stranded remainder isn't being paid off
  separately by choice — it'll clear via the existing payment under the
  Consumer Credit Act's payment-allocation rule (surplus above minimum
  goes to the highest-interest balance first), at negligible interest cost.
- **Capital One / Tesco** — untouched, no action needed.
