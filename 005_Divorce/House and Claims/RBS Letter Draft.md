...
Finally, please update your records with the contact details below, so anything sent by post, phone or message reaches us both.
...
- **~2,000 characters / 330 words** — check the live character counter in the RBS app before
  sending; may need trimming or splitting into "1/2" and "2/2" messages if the secure-message
  channel has a lower limit (RBS's own limit isn't published anywhere findable).
- **Channel:** secure message via RBS Digital Banking (Rupert has app access) — no general RBS
  mortgage email address exists.
- **Send only after Heather has seen it** (per the 7 Jul agreement) and, per the sequencing fix
  (23 Jul), only using an out-of-NatWest-Group lender for the £7,500 loan — this letter itself is
  no longer gated on the loan completing.
